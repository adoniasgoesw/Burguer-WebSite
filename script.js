document.addEventListener('DOMContentLoaded', function() {
    ...
    function addToCart(name, price, cartRef = cart) {
        const existingItem = cartRef.find(item => item.name === name);
    ...
    function removeItemCart(name, cartRef = cart) {
        const index = cartRef.findIndex(item => item.name === name);
      
        if (index !== -1) {
          const item = cartRef[index];
          if (item.quantity > 1) {
            item.quantity -= 1;
          } else {
